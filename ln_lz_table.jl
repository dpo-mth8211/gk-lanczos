...
function pr(X)
  Pr = []
  push!(Pr, [x[1:320] for x in X[1]])
  push!(Pr, [x[1:712] for x in X[2]])
  push!(Pr, [x[1:320] for x in X[3]])
  push!(Pr, [x[1:712] for x in X[4]])
  return Pr
end
...
df[:, :"||r0-s||"] = vcat(round.(r0_minr,sigdigits=3), round.(r0_minrq,sigdigits=3), round.(r0_mina,sigdigits=3), round.(r0_sym,sigdigits=3))
df[:, :"||rk-s||"] = vcat(round.(rk_minr,sigdigits=3), round.(rk_minrq,sigdigits=3), round.(rk_mina,sigdigits=3), round.(rk_sym,sigdigits=3))
df[:, :"||r0||"] = vcat(round.(pr0_minr,sigdigits=3), round.(pr0_minrq,sigdigits=3), round.(pr0_mina,sigdigits=3), round.(pr0_sym,sigdigits=3))
df[:, :"||rk||"] = vcat(round.(prk_minr,sigdigits=2), round.(prk_minrq,sigdigits=2), round.(prk_mina,sigdigits=2), round.(prk_sym,sigdigits=2))
df[:, :Status] = vcat(S_minr, S_minrq, S_mina, S_sym)
